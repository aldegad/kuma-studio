---
name: noeuri
description: Run the Noeuri vault audit workflow and repair stale Kuma memory or plan guidance.
user-invocable: true
---
...
- 책임: vault 큐레이션, memory audit, plan checklist 갱신
...
3. source result, 관련 plan, 수정 대상 memory/vault 문서를 읽고 충돌/누락/낡은 지시를 찾는다.
4. 수정이 필요하면 해당 파일을 갱신한다.
5. 최종 result 파일을 완성한 뒤에만 signal 을 touch 한다.
...
## Signal
- /tmp/kuma-signals/noeuri-auto-{task-id}-done
```

### 출력 규칙

- result 파일은 비어 있는 placeholder 로 두지 말고, 최종 내용을 쓴 뒤 signal 을 touch 한다.
...
- memory 파일을 수정할 때는 `name`, `description`, `type` frontmatter 형식을 유지한다.
- vault 또는 memory 를 고쳤다면 `~/.kuma/vault/log.md` 에 `FIX:` 또는 `INGEST:` 성격이 드러나는 엔트리를 append 한다.
- result 파일 작성 완료 전에는 절대 signal 을 touch 하지 않는다.
...
4. 마지막에만 `mkdir -p /tmp/kuma-signals && touch /tmp/kuma-signals/noeuri-auto-{task-id}-done` 를 실행한다.
