...
```bash
npm run kuma-pickerd:set-agent-note -- --author codex --status in_progress --message "Implementing the requested change."
```

### Finished

```bash
npm run kuma-pickerd:set-agent-note -- --author codex --status fixed --message "Updated the picked element and verified the change."
```

### Reselect needed

```bash
npm run kuma-pickerd:set-agent-note -- --author codex --status needs_reselect --message "The saved selection no longer matches the current UI."
```
