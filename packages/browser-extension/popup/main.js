...
function formatCurrentPageSizeSummary(result) {
  const width = Number(result?.currentPageViewport?.width);
  const height = Number(result?.currentPageViewport?.height);
...
async function refreshConnectionState(showFailure = false) {
  ...
  try {
    ...
    setConnectionState({
      ...
      summary: formatCurrentPageSizeSummary(result),
      url: daemonUrl,
      showForm: false,
