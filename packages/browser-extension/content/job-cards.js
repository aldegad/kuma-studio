(() => {
  ...
  function upsertCard(card) {
    ...
    if (existing) {
      const wasDismissed = isCardDismissed(existing.card);
      existing.card = {
        ...existing.card,
        ...card,
      };
      const isDismissedNow = isCardDismissed(existing.card);
      if (!isDismissedNow) {
        dismissedCardIds.delete(card.id);
      }

      // If the card was already on screen and only status/message changed,
      // update its look in place without repositioning.
      const hasExplicitPosition =
        card.position &&
        typeof card.position === "object" &&
        typeof card.position.left === "number" &&
        typeof card.position.top === "number";
      if (
        existing.element.style.display !== "none" &&
        wasDismissed === isDismissedNow &&
        !hasExplicitPosition
      ) {
        updateCardLook(existing);
        return;
      }
    } else {
