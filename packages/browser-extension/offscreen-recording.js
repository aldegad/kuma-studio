    ...
    try {
      ...
      const finalBlob =
        speedMultiplier > 1 && capturedFrames?.length
          ...
          : await KumaPickerExtensionRecordingMedia.finalizeRecordedBlob(blob, {
              durationMs: Date.now() - startedAt,
              mimeType: recorder.mimeType || mimeType || "video/webm",
            });
      const finalDurationMs = KumaPickerExtensionRecordingMedia.estimateWebmDurationMsFromBytes(
        await KumaPickerExtensionRecordingMedia.readBlobAsUint8Array(finalBlob),
      );
      ...
      await chrome.runtime.sendMessage({
        ...
        durationMs: finalDurationMs || Date.now() - startedAt,
        speedMultiplier,
      });
    } finally {
