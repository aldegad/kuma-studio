...
describe("browser extension manifest", () => {
  it("declares loopback websocket permissions, download diagnostics access, and a minimum Chrome version", () => {
    const manifestPath = path.resolve(process.cwd(), "packages/browser-extension/manifest.json");
    const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));

    expect(manifest.minimum_chrome_version).toBe("116");
    expect(manifest.permissions).toEqual(expect.arrayContaining(["downloads", "contentSettings"]));
