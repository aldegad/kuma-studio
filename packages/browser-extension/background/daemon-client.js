...
function buildJobAnchorFromElement(elementData) {
  if (!elementData) {
    return null;
  }

  const anchor = {};
  if (typeof elementData.selectorPath === "string" && elementData.selectorPath.trim()) {
    anchor.selectorPath = elementData.selectorPath.trim();
  }
  if (typeof elementData.selector === "string" && elementData.selector.trim()) {
    anchor.selector = elementData.selector.trim();
  }
  if (elementData.rect && typeof elementData.rect === "object") {
    anchor.rect = {
      x: typeof elementData.rect.x === "number" ? elementData.rect.x : 0,
      y: typeof elementData.rect.y === "number" ? elementData.rect.y : 0,
      width: typeof elementData.rect.width === "number" ? elementData.rect.width : 0,
      height: typeof elementData.rect.height === "number" ? elementData.rect.height : 0,
    };
  }
  if (elementData.pickedPoint && typeof elementData.pickedPoint === "object") {
    anchor.point = {
      x: typeof elementData.pickedPoint.x === "number" ? elementData.pickedPoint.x : 0,
      y: typeof elementData.pickedPoint.y === "number" ? elementData.pickedPoint.y : 0,
    };
  }
...
function createSelectionPayload(pageContext, screenshot) {
  ...
  const jobAnchor = buildJobAnchorFromElement(pageContext.element);
  ...
  return {
    ...
    element,
    elements: Array.isArray(pageContext.multiSelection)
      ? pageContext.multiSelection.map((el) => ({ ...el, snapshot }))
      : [element],
    job:
