...
- In an installed host project, run the host root `agent-pickerd:get-selection` script before inspecting anything else.
...
- Post progress with `npm run agent-pickerd:set-agent-note -- --author gemini --status acknowledged --message "Read the selection and investigating."`
- Update the shared note to `in_progress`, `fixed`, or `needs_reselect` as the task evolves.
- Before replying, leave a final note so the result is visible inside Agent Picker.
